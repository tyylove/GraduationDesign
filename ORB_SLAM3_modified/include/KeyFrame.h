...
public:
    ...
    void SetNotErase();//当前关键帧是否具有不被删除的权限 参与回环检测的具有权限
    void SetErase();

    // Set/check bad flag
    void SetBadFlag(); //执行删除关键帧
    bool isBad();
...
protected:
    ...
    std::vector<MapPoint*> mvpMapPoints;//关键帧地图点保存
    ...
    std::map<KeyFrame*,int> mConnectedKeyFrameWeights;//当前关键帧的共视关键帧及其权重
    std::vector<KeyFrame*> mvpOrderedConnectedKeyFrames;//所有共视关键帧，按权重大小排序
    ...
    std::set<KeyFrame*> mspLoopEdges;//与当前帧形成回环的关键帧集合
    ...
    // Bad flags
    bool mbNotErase;
    bool mbToBeErased;//当前关键帧是否曾被豁免删除
    bool mbBad;    //标记是否为坏帧
