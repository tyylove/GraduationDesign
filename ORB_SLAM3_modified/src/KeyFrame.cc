...
int KeyFrame::TrackedMapPoints(const int &minObs)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad()) //依次检查该地图点物理上和逻辑上是否删除，若删除，操作
            {
                if(bCheckObs)
                {
...
{
//遍历当前地图点获取与其他关键帧的共视程度 成功调用关键帧后创建生成树
    ...
    for(vector<MapPoint*>::iterator vit=vpMP.begin(), vend=vpMP.end(); vit!=vend; vit++)
    {
        ...
        {
            if(mit->first->mnId==mnId || mit->first->isBad() || mit->first->GetMap() != mpMap)//与当前关键帧本身不算共视
                continue;
            KFcounter[mit->first]++;
...

    {
        ...
        {
            mpParent = mvpOrderedConnectedKeyFrames.front();//生成树
...
void KeyFrame::SetBadFlag()//先标记再清除，默认置为False
{
    {
        unique_lock<mutex> lock(mMutexConnections);//是否具有豁免特权
        if(mnId==mpMap->GetInitKFid())
        {
            return;
        }
        else if(mbNotErase)//有特权
        {
            mbToBeErased = true;
            return;
        }
...
//删除当前关键帧中地图点对本帧的观测
    ...
    {//删除共视图
        ...
        set<KeyFrame*> sParentCandidates;//父节点候选
        if(mpParent)
