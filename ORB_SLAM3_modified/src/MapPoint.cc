...
void MapPoint::AddObservation(KeyFrame* pKF, int idx)	//向参考帧pKF中添加对本地图点的观测，本地图的标号为idx
{
    unique_lock<mutex> lock(mMutexFeatures);
...
void MapPoint::EraseObservation(KeyFrame* pKF)
{
    ...
    {
        unique_lock<mutex> lock(mMutexFeatures);
        //查找要删除的观测，根据单目双目类型的不同从其中删除当前地图点被观测到的次数
        if(mObservations.count(pKF))
        {
            ...
            // If only 2 observations or less, discard point观测点少于2时，舍弃该点
            if(nObs<=2)
                bBad=true;
        }
    }
//告知可以观测的该MapPoint的Frame,该MapPoint已被删除
    if(bBad)
        SetBadFlag();
}
...
void MapPoint::SetBadFlag()//删除地图点
{
...
void MapPoint::Replace(MapPoint* pMP)//地图点的替换
{
...
{    //step1获取地图点的相关信息
    ...
    if(observations.empty())
        return;
//step2 根据观测到但前地图点的关键帧取平均计算平均观测方向
