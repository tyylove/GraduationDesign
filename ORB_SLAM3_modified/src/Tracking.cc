    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    if(settings){
        ...
        cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);//strSettingPath 配置文件路径
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)//每传来一帧图片，就调用一次这个函数
{
